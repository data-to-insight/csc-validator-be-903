...
@rule_definition(
    code="710",
    message="Child with a DoLO can not be placed in secure children's homes, with parents, young offender institutions, or supported accommodation.",
    affected_fields=["DOLO_START", "PLACEMENT"],
    tables=["DoLo", "Episodes"],
)
def validate(dfs):
    # IIf <DOLO_START> and <DOLO_END> are present then <PLACEMENT>
    # for any episode where <DECOM> and <DEC> overlap the DOLO must
    # not be 'K1', 'P1', 'R5', 'K3'.
    if "DoLo" not in dfs:
        return {}
    ...
    wrong_placement_overlaps = overlaps[
        overlaps["PLACEMENT"].isin(["K1", "P1", "R5", "K3"])
    ]
