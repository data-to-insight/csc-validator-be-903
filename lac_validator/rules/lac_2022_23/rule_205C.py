...
def validate(dfs):
    ...
    neveruasc = (
        (allmerged[["UASC", "UASClast"]] == "0")
        | allmerged[["UASC", "UASClast"]].isna()
    ).all(axis=1)
    hasduc = allmerged[["DUC", "DUClast"]].notna().any(axis=1)

    errormask = neveruasc & hasduc
    ...
    print(
        pd.concat(
            (allmerged, pd.DataFrame({"u": neveruasc, "d": hasduc})), axis=1
        ).tostring()
    )
