...
def validate():
    ...
    def _validate(dfs):
        if ("PlacedAdoption" not in dfs) or ("Episodes" not in dfs):
            ...
            adoption_eps = episodes[
                episodes["PLACE"].isin(["A3", "A4", "A5", "A6"])
            ].copy()
            ...
            placed_adoption_inds = placed_adoption.groupby("CHILD")[
                "DATE_PLACED"
            ].idxmax(skipna=True)
            ...
            adoption_eps_inds = adoption_eps.groupby("CHILD")["DECOM"].idxmin(
                skipna=True
            )
