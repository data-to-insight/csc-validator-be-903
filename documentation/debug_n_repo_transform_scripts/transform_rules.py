...
def transform_rule(rule_file_name):
    ...
    subvalidate_end = validate.rfind(
        "return"
    )  # finds the outer last return statement such that it can be excluded.

    subvalidate_start = validate.find("def _validate(dfs)")
    subvalidate = validate[subvalidate_start:subvalidate_end].replace(
        "_validate", "validate"
    )
    ...
    rule_def = (
        rule_def.replace("error = ErrorDefinition", "@rule_definition")
