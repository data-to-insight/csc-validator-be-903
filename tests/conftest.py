...
def dummy_chscp():
    ...
    combined_fake_dir = os.path.join(os.path.dirname(__file__), "fake_data", "combined_scp_ch_fake.xlsx")
    dummy_ch = read_file(fake_ch_dir)
    dummy_scp = read_file(fake_scp_dir)
    dummy_combined = read_file(combined_fake)
    return dummy_ch, dummy_scp, fake_ch_dir, fake_scp_dir, combined_fake_dir, dummy_combined
