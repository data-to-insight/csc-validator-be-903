...
def test_all_configured_error_definitions():
    ...
    for rule_code, rule in registry.items():
        ...
        assert isinstance(rule.code, str), f'The error code {rule.code} is not a string!'
        assert isinstance(rule.message, str), f'The error description {rule.message} is not a string!'
        assert isinstance(rule.affected_fields, list), f'The affected fields {rule.affected_fields} is not a list!'
        assert all(isinstance(f, str) for f in rule.affected_fields), f'Not all fields in affected_fields are strings!'
...
def test_all_configured_error_functions(data_choice, dummy_empty_input, dummy_input_data):
    ...
    for rule_code, rule in registry.items():
        ...
        for table_name, issue_list in result.items():
            ...
            assert isinstance(issue_list, list), f'Returned list of error locations {issue_list} is not a list (its a {type(issue_list)})!'
            for issue_location in issue_list:
                assert issue_location in dummy_data_copy[table_name].index, f'Location {issue_location} not found in {table_name} index - check returned locations!'
...
def test_has_correct_table_names(dummy_input_data):
    for rule_code, rule in registry.items():
        ...
        if rule.code.lower() not in errors_skipped_for_csv:
            assert len(result) > 0, f'Validator for {rule.code} does not appear to operate on any configured table names - check spelling!'
        assert all(r in dummy_input_data_copy for r in result), f'Validator for {rule.code} returns a wrong table name!'
